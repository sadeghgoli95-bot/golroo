...
import { buildScientificPrompt } from "../prompts/scientific";

const AI_SUGGESTION_CONFIDENCE = 50;

export async function analyzeScientific(
  ...
  const completion = await provider.complete(buildSingleUserMessageRequest(buildScientificPrompt(article)));

  return {
    scores: {},
    warnings: [],
    errors: [],
    suggestions: [{ id: "scientific-ai", message: completion.content, priority: "medium" }],
    improvements: [],
    priorityActions: [],
    statistics: {},
